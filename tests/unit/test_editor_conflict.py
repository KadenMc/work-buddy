"""Unit tests for EditorConflict + write_file_raw retry behavior.

Covers:
- EditorConflict is classified as transient (so the retry queue picks it up)
- write_file_raw retries on 409 per the 5/10/20s schedule
- write_file_raw raises EditorConflict on retry exhaustion
- write_file_raw returns False (not raises) on non-409 failures so
  bridge-down still goes through the existing fallback logic
...
class TestWriteFileRawRetryBehavior:
    ...
    def test_success_after_one_409(self):
        # First call returns 409 (editor dirty), second returns 200.
        # We need to also patch sleep so the test doesn't actually wait 5s.
        with patch("work_buddy.obsidian.bridge.time.sleep") as mock_sleep, \
             self._patch_status(
                 (409, {"error": "editor_dirty"}),
                 (200, {"path": "x.md", "created": False}),
             ):
            assert write_file_raw("x.md", "content") is True
            # Should have slept exactly 5s before the second attempt.
            mock_sleep.assert_called_once_with(5)

    def test_raises_editor_conflict_after_full_schedule(self):
        # All 4 attempts (immediate + 5s + 10s + 20s) hit 409.
        with patch("work_buddy.obsidian.bridge.time.sleep") as mock_sleep, \
             self._patch_status(
                 (409, {"error": "editor_dirty"}),
                 (409, {"error": "editor_dirty"}),
            ...
            # The schedule should be 5, 10, 20 (no sleep before first attempt).
            assert [c.args[0] for c in mock_sleep.call_args_list] == [5, 10, 20]

    def test_network_failure_returns_false_no_retry(self):
        """Bridge down → status is None. Don't waste 35s on the
        editor-conflict schedule; let the higher-layer fallback kick in."""
        with patch("work_buddy.obsidian.bridge.time.sleep") as mock_sleep, \
             self._patch_status((None, None)):
            assert write_file_raw("x.md", "content") is False
            mock_sleep.assert_not_called()

    def test_other_4xx_returns_false_no_retry(self):
        """A 400 / 404 / 500 isn't a transient conflict — don't retry."""
        with patch("work_buddy.obsidian.bridge.time.sleep") as mock_sleep, \
             self._patch_status((400, {"error": "bad request"})):
            assert write_file_raw("x.md", "content") is False
            mock_sleep.assert_not_called()
