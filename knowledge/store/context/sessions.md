...
The skills below are the **leaf-level building blocks** for reading session contents once a session id is in hand:
...
Session ID resolution: canonical IDs preserve the provider's native session identity. Built-in Claude and Codex IDs are UUIDs, and all session_* skills accept unique partial prefixes (8 chars is the conventional display form). Three helpers share the same resolver:
