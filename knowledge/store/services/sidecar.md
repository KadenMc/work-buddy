...
dev_notes: |-
  ...
  This deliberately **dropped** the previous count-based shift (`.1→.2→.3→.4`, drop `.4`) and the `_SERVICE_LOG_BACKUP_COUNT` constant. That scheme only weighed the *live* file and never enforced a size/age bound on already-rolled backups, so a low-volume service's oversized backup persisted indefinitely (a 160 MB `messaging.1.log` survived three weeks — task t-f54ef1ac). Timestamped (dateext) naming + age-reaping by the `service-logs` artifact (`MtimeWindow(7d)`, pinning the live `<name>.log`) replaces it, mirroring logrotate's `dateext` + `maxage` split.
