...
* **Frontend:** the UI is generated by Python modules under ``frontend/`` and served as content-hashed cached assets. See ``services/dashboard/frontend`` for the module structure, concatenation order, the event-delegation interaction model, escaping, the shared pager and filters, asset serving, and the add-a-tab pattern.
