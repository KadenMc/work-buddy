...
1. Dashboard endpoint POSTs through ``_reject_read_only()``, calls ``conversations.store.create_conversation(...)`` directly (NOT the ``conversation_create`` skill — that fires a CHAT toast and a workflow-view tab via ``_notify_conversation_created``, double-mounting the conversation). The seed message is added with ``message_type='question'`` and ``response_type='freeform'`` so the spawned agent's ``conversation_poll`` returns the user's first reply directly — without this, ``conversation_poll`` returns ``no_pending_question`` and the agent sends a duplicate greeting.
