...
dev_notes: |-
  ...
  ## Inner-thread action switcher (fill-or-refine draft)

  The per-source action switcher (`scripts/tabs/threads/actions.py:_renderActionSwitcher`) renders on a child thread's right-pane editor. Its options come from `window._groupState.actionOptionsByUmbrella[parentId]`; a child opened directly lazy-fetches `GET /api/threads/<id>/action_options` (backed by `service._resolve_action_library_for_thread`, which resolves the library from the thread's own `inciting_event_summary` source), which now also carries each action's parameter schema (`service._attach_param_schemas` → `_project_param_schema`, with `required` flags). The `/groups` endpoint stays group-umbrella-only; `/action_options` is the any-thread library surface.

  Selecting an action does NOT write to the server. It sets an uncommitted client draft (`window._groupState.draftByThread[threadId] = {skill_name, params, message}`) and `renderActionInRightPane` short-circuits to `renderActionDraft`: a pending-swap banner, blank REQUIRED fields (from the schema), and two commit paths. Approve (enabled only when required fields are filled) POSTs `/accept` with `{action: {skill_name, parameters}}`; the "Issue? Hand it back for refinement" group POSTs `/redirect_action` with `{target_action, params (seeds), feedback}`. The draft lives in `_groupState` (not the thread-detail cache), so an SSE refresh re-renders without wiping it. The message is bound to Redirect: clicking Approve with a non-empty note warns instead of silently dropping it.
---
