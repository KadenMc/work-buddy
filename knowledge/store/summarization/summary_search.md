...
**Empty results are not proof of absence.** Session summaries are generated by a background pipeline that a user may have opted out of (`conversation_summaries` preference), that stays dormant without a usable LLM backend, and that only covers pre-existing history after a one-off `summarization_backfill`. Zero hits for a session-shaped query can therefore mean "not summarized yet" rather than "no such session" — fall back to `context_search(source="conversation")`, which searches raw transcripts and works regardless of the summary pipeline's state. See `summarization/failure-handling` for the pipeline's states.
