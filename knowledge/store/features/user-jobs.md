...
3. **Dashboard assisted form** — choose **Help me shape this** in that same widget. After opt-in and provider/model disclosure, the shared conversation component suggests typed patches that fill the visible fields. User edits win conflicts; changes have receipts and conditional Undo. Only the user's **Create job** click submits. Chat, including a message saying "yes", cannot schedule or overwrite a job. See `services/dashboard/react/assisted-drafts`.
