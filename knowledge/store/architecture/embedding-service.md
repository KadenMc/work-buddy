...
dev_notes: |-
  ...
  **Three-signal fusion is fragile without alias coverage.** Skills with zero authored aliases lose ground to aliased competitors under RRF (three votes vs. two). For the rework to be a net win, every skill needs >=4 good aliases — see `registry.py` and the embedding-brittleness report (`data/report/*-phase4.md`). The baseline brittleness experiment, the post-rework regression, and the post-alias-backfill win are all archived under `data/report/*knowledge-search-brittleness*`.
