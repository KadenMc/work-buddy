...
The **MCP gateway** is a framework consumer too: every `wb_run` capability dispatch runs through `guarded_call` under a `wb_run:<capability>` operation key (`work_buddy/mcp_server/dispatch_resilience.py`). It composes a `TimeoutStrategy` only when a capability opts in by declaring a finite `timeout_seconds` (a scalar or a `(params) -> seconds` policy); the gateway timeout is opt-in, so an undeclared budget is unbounded and no flat default is auto-applied (a too-low default would cap deliberately-long-running capabilities — human-in-the-loop polls, retry wrappers, LLM submission). For capabilities requiring any bridge-backed tool (the Obsidian bridge or an in-Obsidian plugin that depends on it — `datacore`, `smart_connections`, `google_calendar`; see `tools.obsidian_backed_tools()`), it also composes a shared `obsidian_bridge` `CircuitBreakerStrategy`. The gateway registers the process's telemetry listeners at startup (an `InMemoryMetrics` recorder plus a logging listener), so every guarded call — gateway dispatch and `@bridge_retry` alike — records metrics and logs a `guard.*` line.
