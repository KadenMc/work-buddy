...
Truth state is intentionally scoped differently: each participating project or purpose root owns a `.wb-truth/` sidecar containing its profile, permanent identity, SQLite ledger, optional blobs, and deterministic recovery export. These stores live beside the material they describe rather than under the shared `<data_root>`. Only the machine inventory lives centrally at `<data_root>/db/truth_registry.db`. The registry tracks path, identity, profile, title, last-seen time, and reachability for discovery and portable backup coverage. It is not a claim store. See `architecture/truth`.
