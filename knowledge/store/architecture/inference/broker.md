...
``work_buddy.inference.parse_priority(value)`` maps a case-insensitive name string (``"interactive"`` / ``"workflow"`` / ``"background"``) — or a ``Priority`` / ``None`` passthrough — onto the enum. It exists for the MCP boundary, where skill params (``llm_call`` / ``llm_submit``) arrive as JSON strings and must map onto the enum before reaching the broker.
