...
- dev
dev_notes: |-
  The `scan` step runs a hybrid matcher: BM25 + dense retrieval fused with Reciprocal Rank Fusion, with the scored substring-grep matcher (`_match_units_via_grep`) as a lexical fallback. Queries include changed paths/slugs and one docstring per changed Python file. Keep them batched per model: per-file `search()` calls multiply embedding round-trips and can exhaust the workflow budget. `_QUERY_EMBED_TIMEOUT_S` bounds query requests; it does not bound construction of a cold local dense index.

  When `index.enabled` is true, `_search_units_via_consolidated` calls `embedding.client.index_search_many` once (`POST /index/search_many`, partition `knowledge`, `filters={"scope":"system"}`). It sets `warm_retry=False` so a cold resident matrix cannot add a sleep and a second 90s request. Available lexical results remain usable while the matrix warms. A failed request, invalid result shape, or empty/stale partition goes directly to the scored grep fallback, followed by canonical entry-point force-inclusion. It never starts a local dense build as recovery. When consolidation is disabled, the normal in-process `search_many` path remains available, including its query-time BM25 degradation and hard-error grep fallback.

  The `auto_run` timeout is 90s. The consolidated request budget is 25s, leaving room for subprocess startup, store loading, and lexical matching. The nonconsolidated path can still pay for a cold knowledge-index build; its query timeout does not cover that construction. Every auto-run imports the callable in a fresh subprocess, so changes to `scan_changes` take effect on the next run without restarting the gateway.
