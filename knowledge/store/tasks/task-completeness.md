...
- `session_evidence` — per session (across created/assigned/developed/**note_reader** roles): commits, writes, summary. Heed each `note` and the top-level `cache_note`. A `note_reader` role means the session demonstrably read the native task/Co-work document through `task_read`, `task_assign`, or the document surface; frozen Markdown-path reads count only as historical evidence. This is the Rung-3 surface for "read it, did the work, never referenced the task id in a commit." Pure-triage reads are pruned; the ones that survive carry work.
