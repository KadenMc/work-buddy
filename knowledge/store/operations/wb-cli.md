...
- `wbuddy harness list [--json]` -- list supported agent-host harnesses (`claudecode`, `codexcli`), their rulesync target ids, feature projection, browser surface, and selection state.
...
- `wbuddy dashboard --read-only --port <port>` -- serve a separate foreground dashboard on loopback for inspection. The explicit port must be outside the normal service ports (5124, 5126, 5127). This mode installs the process read-only guard before loading the dashboard; it does not open a browser automatically. See `services/dashboard` for the guard and supported inspection behavior.
