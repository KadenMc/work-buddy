"""``calendar`` context source — wraps the legacy Google Calendar collector."""

from __future__ import annotations

from typing import Any

from work_buddy.context.sources._markdown_wrapper import MarkdownCollectorSource
from work_buddy.context import registry as _registry


class CalendarSource(MarkdownCollectorSource):
    name = "calendar"
    _heading = "Calendar"
    _default_cfg: dict[str, Any] = {}

    def __init__(self):
        try:
            from work_buddy.collectors import calendar_collector
            self._collect_fn = calendar_collector.collect
        except Exception:
            self._collect_fn = None


_registry.register(CalendarSource())
