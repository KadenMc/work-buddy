...
def _spawn_investigation_job(rule: dict, latest_path: str) -> str:
    ...
    # The scheduler reads `prompt` from the markdown BODY (line 183 of
    # work_buddy/sidecar/scheduler/jobs.py: ``prompt=body.strip()``),
    # NOT from a `prompt:` field in frontmatter. Putting the prompt
    # inside the frontmatter results in an empty body → empty prompt →
    # executor returns "Empty prompt." with status=error and no log.
    frontmatter = (
        "---\n"
