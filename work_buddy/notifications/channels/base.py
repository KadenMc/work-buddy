...
A channel is a delivery mechanism that can send notifications to the user
and (for request-type notifications) collect responses. Each channel
...
class NotificationChannel(ABC):
    ...
    @abstractmethod
    def name(self) -> str:
        """Channel identifier (e.g., 'obsidian', 'telegram')."""
        ...

    @property
    @abstractmethod
    def supported_response_types(self) -> set[ResponseType]:
        """Which response types this channel can handle.

        The notification system uses this to decide which channels
    ...
    def supports_custom_ui(self) -> bool:
        """Whether this channel supports generative/custom UI templates.

        If True, the channel can render arbitrary UI from a template
    ...
    def is_available(self) -> bool:
        """Check if the channel is currently reachable.
    ...
    def poll_response(self, notification_id: str) -> StandardResponse | None:
        ...
        if still pending. For channels with push support (Telegram webhooks),
        this may read from a local cache that the webhook populated.

        The response is standardized regardless of channel-specific UI:
