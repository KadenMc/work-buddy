"""work-buddy ``wbuddy`` command-line interface.
...
# `wbuddy` runs standalone (a plain user shell, not a Claude Code session), so it
...
# session id when an agent shells out to `wbuddy`.
os.environ.setdefault("WORK_BUDDY_SESSION_ID", "wbuddy-cli")
