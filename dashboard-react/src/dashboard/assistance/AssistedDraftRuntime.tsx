...
interface ReceiptRecord {
  ...
  /** Exact causal turns supplied by the assistance service; not patch authority. */
  readonly sourceMessageId?: string;
  readonly replyMessageId?: string;
}

interface RecentAssistedChange {
  readonly token: number;
  readonly patchId: string;
  readonly changes: readonly FieldChange[];
}

type ReceiptAnchors = Pick<ReceiptRecord, "sourceMessageId" | "replyMessageId">;

interface AssistanceRuntime {
  readonly activeKey: string | null;
  readonly receipts: ReadonlyMap<string, readonly ReceiptRecord[]>;
  readonly recentChanges: ReadonlyMap<string, RecentAssistedChange>;
...
const ASSISTED_FIELD_HIGHLIGHT_MS = 4000;
const MESSAGE_ID_PATTERN = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,255}$/;
const messageId = (value: unknown): string | undefined => typeof value === "string" && MESSAGE_ID_PATTERN.test(value) ? value : undefined;

function ReceiptCards({ records, editable, onReview }: {
  readonly records: readonly ReceiptRecord[];
  readonly editable: boolean;
  readonly onReview?: (record: ReceiptRecord, action: "apply" | "reject" | "undo") => void;
}) {
  if (records.length === 0) return null;
  return <div className="wb-assistance-receipts" aria-label="Assistant field changes">
    {records.map((record) => <section key={record.patch.patchId} className="wb-assistance-receipt" aria-label="Assistant patch receipt">
      ...
      {onReview && <div className="wb-assistance-actions">
        {record.receipt.pendingFields.length > 0 && record.receipt.status !== "undone" && <>
          <Button type="button" disabled={!editable} onClick={() => onReview(record, "apply")}>Apply suggestions</Button>
          <Button type="button" disabled={!editable} onClick={() => onReview(record, "reject")}>Dismiss suggestions</Button>
        </>}
        {record.changes.length > 0 && record.receipt.status !== "undone" && <Button type="button" disabled={!editable} onClick={() => onReview(record, "undo")}>Undo assistant changes</Button>}
      </div>}
    </section>)}
  </div>;
}
...
export function useAssistedDraft<Value>(draftName: string, draft: WidgetDraftHandle<Value>, options: UseAssistedDraftOptions): AssistedDraftControl {
  ...
  const recent = runtime?.recentChanges.get(identityKey);
  return {
    ...
    fieldProps: (path) => {
      ...
      const applied = recent?.changes.some((change) => pathKey(change.path) === key && equalJson(readField(draft.value, path), change.after)) ?? false;
      return {
        ...
        "aria-description": pending ? "Assistant suggestion awaiting review in the assistance panel." : applied ? "Recently filled by assistant. This highlight clears automatically; you can edit or undo this change." : undefined,
      };
    },
  };
}
...
export function AssistedDraftRuntimeProvider({ children, fetchImpl }: { readonly children: ReactNode; readonly fetchImpl?: typeof fetch }) {
  ...
  const [recentChanges, setRecentChanges] = useState<ReadonlyMap<string, RecentAssistedChange>>(new Map());
  const recentChangeTimers = useRef(new Map<string, ReturnType<typeof setTimeout>>());
  const recentChangeSequence = useRef(0);
  const [, rerender] = useState(0);
  const clearRecentChange = useCallback((key: string) => {
    const timer = recentChangeTimers.current.get(key);
    if (timer !== undefined) clearTimeout(timer);
    recentChangeTimers.current.delete(key);
  ...
  const markRecentChange = useCallback((key: string, patchId: string, changes: readonly FieldChange[]) => {
    if (changes.length === 0) return;
    const priorTimer = recentChangeTimers.current.get(key);
    if (priorTimer !== undefined) clearTimeout(priorTimer);
    const token = ++recentChangeSequence.current;
    setRecentChanges((current) => new Map(current).set(key, { token, patchId, changes }));
    recentChangeTimers.current.set(key, setTimeout(() => {
      recentChangeTimers.current.delete(key);
      setRecentChanges((current) => {
    ...
    }, ASSISTED_FIELD_HIGHLIGHT_MS));
  }, []);
  useEffect(() => () => {
    for (const timer of recentChangeTimers.current.values()) clearTimeout(timer);
    recentChangeTimers.current.clear();
  }, []);
  ...
  const reset = useCallback((key: string) => {
    ...
    clearRecentChange(key);
  }, [clearRecentChange, revocations]);
  ...
  const value = useMemo(() => ({ activeKey, receipts, recentChanges, register, changed, reset, open }), [activeKey, receipts, recentChanges, register, changed, reset, open]);
  ...
  const dock = binding ? {
    ...
    content: <AssistantDock key={binding.key} binding={binding} open={activeKey === binding.key} initialRecords={receipts.get(binding.key) ?? []} client={client} revocations={revocations} pauses={pauses} onClose={close} onRecords={onRecords} onRecentChange={markRecentChange} onSession={onSession} />,
  } : null;
...
function AssistantDock({ binding, open, onClose, onRecords, onRecentChange, onSession, client, revocations, pauses, initialRecords }: {
  readonly binding: LiveDraft;
  readonly open: boolean;
  readonly onClose: () => void;
  readonly onRecords: (key: string, records: readonly ReceiptRecord[]) => void;
  readonly onRecentChange: (key: string, patchId: string, changes: readonly FieldChange[]) => void;
    ...
    for (const record of retained) {
      try {
        ...
        recovered.push({
          ...
          receipt,
          stage: "committed",
          sourceMessageId: messageId(record.sourceMessageId),
          replyMessageId: messageId(record.replyMessageId),
        });
  ...
  const commitPlan = async (patch: AssistedDraftPatch, plan: PatchPlan, priorChanges: readonly FieldChange[] = [], anchors: ReceiptAnchors = {}) => {
    if (!canMutate()) return;
    ...
    if (plan.changes.length) {
      ...
      retain({ patch, receipt, changes, stage: "prepared", ...anchors });
    ...
    publish({ patch, receipt, changes, ...anchors });
    const newlyApplied = plan.changes.filter((change) => receipt.appliedFields.some((path) => equalJson(path, change.path)));
    if (receipt.status !== "undone" && receipt.status !== "rejected" && newlyApplied.length > 0) {
      onRecentChange(binding.key, patch.patchId, newlyApplied);
    }
  ...
  const applyPatch = async (unknownPatch: unknown, serverReceipt: DraftPatchReceipt | null, expectedEpoch: number, anchors: ReceiptAnchors) => {
    ...
    if (local) {
      const anchored: ReceiptRecord = {
        ...local,
        sourceMessageId: anchors.sourceMessageId ?? local.sourceMessageId,
        replyMessageId: anchors.replyMessageId ?? local.replyMessageId,
      };
      const anchorsChanged = anchored.sourceMessageId !== local.sourceMessageId || anchored.replyMessageId !== local.replyMessageId;
      if (serverReceipt && (serverReceipt.resultingRevision > local.receipt.resultingRevision || serverReceipt.status === "undone" || serverReceipt.status === "rejected")) {
        if (!equalJson(local.receipt, serverReceipt) || anchorsChanged) publish({ ...anchored, receipt: serverReceipt, changes: [] });
        return;
      }
      if (anchorsChanged) publish(anchored);
    ...
    if (serverReceipt) {
      ...
      publish({ patch, receipt: serverReceipt, changes: [], ...anchors });
      return;
    }
    await binding.flush();
    if (!canMutate() || expectedEpoch !== authorityEpoch.current) return;
    ...
    await commitPlan(patch, plan, [], anchors);
  };
  ...
  pollRef.current = async () => {
    ...
    try {
      ...
      for (const entry of patches) {
        ...
        const anchors = { sourceMessageId: messageId(entry.sourceMessageId), replyMessageId: messageId(entry.replyMessageId) };
        operationChain.current = operationChain.current.catch(() => undefined).then(() => applyPatch(entry.patch, entry.receipt as DraftPatchReceipt | null, expectedEpoch, anchors));
  ...
  const review = (record: ReceiptRecord, action: "apply" | "reject" | "undo") => {
    operationChain.current = operationChain.current.catch(() => undefined).then(async () => {
      ...
      if (action === "reject") {
        ...
        await commitPlan(record.patch, planUndo(record.patch.patchId, record.changes, current, binding.focused()), [], { sourceMessageId: record.sourceMessageId, replyMessageId: record.replyMessageId });
      } else {
        ...
        await commitPlan(record.patch, { ...plan, receipt: { ...plan.receipt, appliedFields: [...record.receipt.appliedFields, ...plan.receipt.appliedFields] } }, record.changes, { sourceMessageId: record.sourceMessageId, replyMessageId: record.replyMessageId });
      }
    }).catch((reason) => { if (alive.current) setError(failure(reason)); });
  };

  const conversationRecords = conversationId === null ? [] : records.filter((record) => record.patch.conversationId === conversationId);
  const visibleMessageIds = new Set(chatMessages.map((message) => message.id));
  const recordsByMessage = new Map<string, ReceiptRecord[]>();
  const fallbackRecords: ReceiptRecord[] = [];
  for (const record of conversationRecords) {
    // A reply may arrive one poll after its patch. Keep the event beside the
    // source turn until that exact durable assistant reply is visible, then
    // move it to the reply without replaying the form mutation.
    const anchor = record.replyMessageId && visibleMessageIds.has(record.replyMessageId)
      ? record.replyMessageId
      : record.sourceMessageId && visibleMessageIds.has(record.sourceMessageId)
        ? record.sourceMessageId
        : undefined;
    if (!anchor) fallbackRecords.push(record);
    else recordsByMessage.set(anchor, [...(recordsByMessage.get(anchor) ?? []), record]);
  }
  const receiptExtensionRevision = conversationRecords.map((record) => [
    record.patch.patchId,
    record.receipt.status,
    record.receipt.resultingRevision,
    record.sourceMessageId ?? "",
    record.replyMessageId ?? "",
    record.receipt.appliedFields.length,
    record.receipt.pendingFields.length,
  ].join(":")).join("|");
  const appendix = <>
    <VisuallyHidden role="status" aria-live="polite">{conversationRecords[conversationRecords.length - 1]?.receipt.message}</VisuallyHidden>
    {fallbackRecords.length > 0 && <details className="wb-assistance-receipts-fallback">
      <summary>Field change history ({fallbackRecords.length})</summary>
      <ReceiptCards records={fallbackRecords} editable={editable} onReview={review} />
    </details>}
  </>;
  ...
  return <aside ref={paneRef} id={binding.paneId} className="wb-assistance-dock" aria-label="Draft assistance" hidden={!open} inert={!open ? true : undefined}>
    ...
    <div className="wb-assistance-dock__body">
      {session && provider ? <ConversationChat
        ...
        prepareSend={prepareSend} onMessagesChange={observeMessages}
        renderMessageAccessory={(message) => <ReceiptCards records={recordsByMessage.get(message.id) ?? []} editable={editable} onReview={review} />}
        transcriptAppendix={appendix} transcriptExtensionRevision={receiptExtensionRevision}
        ...
        composerPrimaryAction={launchRequired ? {
          ...
          help: { summary: "Launch AI help for this form.", details: "Shares the disclosed fields and recent chat with the selected model. In Jobs, the assistant may also inspect the registered skill and workflow metadata already shown by the form. It can suggest field edits, but only you can submit. Launch does not send any unsent message." },
        } : undefined}
    ...
    {historicalSessions.length > 0 && <details className="wb-assistance-history" open={historyOpen} onToggle={(event) => setHistoryOpen(event.currentTarget.open)}>
      ...
      {historyOpen && historicalSessions.map((previous) => <HistoricalConversation key={previous.assistantSessionId} session={previous} client={client} records={records.filter((record) => record.patch.conversationId === previous.conversationId)} editable={editable} onReview={review} />)}
    </details>}
  </aside>;
}

/** Explicit historical inspection still uses the canonical, read-only Chat. */
function HistoricalConversation({ session, client, records, editable, onReview }: {
  readonly session: AssistanceSession;
  readonly client: AssistanceClient;
  readonly records: readonly ReceiptRecord[];
  readonly editable: boolean;
  readonly onReview: (record: ReceiptRecord, action: "apply" | "reject" | "undo") => void;
  ...
  const unanchored = records.filter((record) => !record.replyMessageId && !record.sourceMessageId);
  const revision = records.map((record) => `${record.patch.patchId}:${record.receipt.status}:${record.replyMessageId ?? record.sourceMessageId ?? ""}`).join("|");
  return <div className="wb-assistance-history__conversation"><ConversationChat
    provider={provider} conversationId={session.conversationId} title="Previous AI help conversation"
    readOnlyReason="This history is preserved. Launch a new session to continue with the form."
    renderMessageAccessory={(message) => <ReceiptCards records={records.filter((record) => (record.replyMessageId ?? record.sourceMessageId) === message.id)} editable={editable} onReview={onReview} />}
    transcriptAppendix={unanchored.length > 0 ? <details className="wb-assistance-receipts-fallback"><summary>Field change history ({unanchored.length})</summary><ReceiptCards records={unanchored} editable={editable} onReview={onReview} /></details> : undefined}
    transcriptExtensionRevision={revision}
  /></div>;
}
